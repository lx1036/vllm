...
allocated = [10,11,12]
block_size = 4
slot_mappings = [torch.arange(seq_len, dtype=torch.long, device="cuda") + block * block_size for block in allocated] # 40 44 48
print(slot_mappings)
'''
...
paged_attention_block_tables[seq_id] = [
    torch.tensor([[block] + [-1]*(max_blocks_per_seq-1)], dtype=torch.int32, device="cuda") for block in allocated
]
