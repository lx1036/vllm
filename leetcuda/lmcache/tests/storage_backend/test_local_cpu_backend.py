...
from leetcuda.lmcache.memory_management import MemoryObj, MemoryFormat, AdHocMemoryAllocator
from leetcuda.lmcache.storage_backend.local_cpu_backend import LocalCPUBackend
from leetcuda.lmcache.tests.utils import create_engine_config, create_engine_key
...
@pytest.fixture
def local_cpu_backend(memory_allocator)-> LocalCPUBackend:
...
def local_cpu_backend_disabled(memory_allocator):
    ...
    config = create_engine_config(local_cpu=False)
    return LocalCPUBackend(config=config, memory_allocator=memory_allocator)
...
class TestLocalCPUBackend:
    # memory_allocator fixture 在 conftest.py 里定义
    ...
    def test_init_with_lookup_server_and_worker(self, memory_allocator):
        ...
        backend = LocalCPUBackend(
            ...
            lmcache_worker=lmcache_worker, # TODO: 这里为啥可以直接传 MockLMCacheWorker() 不报错???
        )
    ...
    def test_init_with_layerwise_config(self, memory_allocator):
        ...
        config = create_engine_config(use_layerwise=True, enable_blending=True)
        backend = LocalCPUBackend(config=config, memory_allocator=memory_allocator)
    ...
    def test_str(self, local_cpu_backend: LocalCPUBackend):
        """Test string representation."""
        assert str(local_cpu_backend) == "LocalCPUBackend"

        local_cpu_backend.memory_allocator.close()

    def test_contains_key_not_exists(self, local_cpu_backend: LocalCPUBackend):
        """Test contains() when key doesn't exist."""
        key = create_engine_key("nonexistent")
        assert not local_cpu_backend.contains(key)
        assert not local_cpu_backend.contains(key, pin=True)

        local_cpu_backend.memory_allocator.close()

    def test_contains_key_exists(self, local_cpu_backend: LocalCPUBackend):
        """Test contains() when key exists."""
        key = create_engine_key("test_key")
    ...
    def test_exists_in_put_tasks(self, local_cpu_backend: LocalCPUBackend):
    ...
    def test_submit_put_task(self, local_cpu_backend: LocalCPUBackend):
    ...
    def test_submit_put_task_reinsert(self, local_cpu_backend: LocalCPUBackend):
    ...
    def test_batched_submit_put_task(self, local_cpu_backend: LocalCPUBackend):
        """Test batched_submit_put_task()."""
        keys = [create_engine_key(f"key_{i}") for i in range(3)]
    ...
    def test_batched_submit_put_task_disabled(self, local_cpu_backend_disabled: LocalCPUBackend):
    ...
    def test_get_blocking_key_not_exists(self, local_cpu_backend: LocalCPUBackend):
    ...
    def test_get_blocking_key_exists(self, local_cpu_backend: LocalCPUBackend):
    ...
    def test_pin_unpin(self, local_cpu_backend: LocalCPUBackend):
        ...
        non_existent_key = create_engine_key("non_existent")
        assert not local_cpu_backend.pin(non_existent_key)
        assert not local_cpu_backend.unpin(non_existent_key)

        local_cpu_backend.memory_allocator.close()

    def test_remove(self, local_cpu_backend: LocalCPUBackend):
        """Test remove()."""
        key = create_engine_key("test_key")
        memory_obj = create_test_memory_obj()
    ...
    def test_allocate(self, local_cpu_backend: LocalCPUBackend):
        """Test allocate()."""
    ...
    def test_allocate_with_format(self, local_cpu_backend: LocalCPUBackend):
    ...
    def test_batched_allocate(self, local_cpu_backend: LocalCPUBackend):
    ...
    def test_get_keys(self, local_cpu_backend: LocalCPUBackend):
        """Test get_keys()."""
        keys = [create_engine_key(f"key_{i}") for i in range(3)]
        memory_objs = [create_test_memory_obj() for _ in range(3)]
    ...
    def test_get_keys_empty(self, local_cpu_backend: LocalCPUBackend):
    ...
    def test_ref_count_management(self, local_cpu_backend: LocalCPUBackend):
