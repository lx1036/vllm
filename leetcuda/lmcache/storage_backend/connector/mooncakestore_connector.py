...
from leetcuda.lmcache.protocol import RemoteMetadata
from leetcuda.lmcache.storage_backend.connector.base_connector import RemoteConnector
from leetcuda.lmcache.storage_backend.local_cpu_storage import LocalCPUBackend
...
# @see  vllm/distributed/kv_transfer/kv_lookup_buffer/mooncake_store.py
...
class MooncakeStoreConfig:
    ...
    @staticmethod
    def load_from_lmcache_config(config: LMCacheEngineConfig) -> "MooncakeStoreConfig":
        """Load config from a file specified in the environment variable."""
...
class MooncakestoreConnector(RemoteConnector):
    def __init__(
        ...
        try:
            # 这么做的好处是，只有运行到这段逻辑才import，这样运行别的代码时不会报错。
        ...
        try:
            ...
            if config_file_path is not None:
               ...
               self.config = MooncakeStoreConfig.load_from_lmcache_config(lmcache_config)
            else:
            ...
            if self.config.storage_root_dir is not None and self.config.storage_root_dir != "":
        ...
        self.register_cpu_buffer()

        logger.info("MooncakeConnector initialized successfully.")

    def register_cpu_buffer(self):
        """Register CPU buffer for zero-copy operations."""
        try:
            allocator = self.local_cpu_backend.memory_allocator
            if hasattr(allocator, "pin_allocator") and hasattr(allocator.pin_allocator, "buffer"):
                buffer = allocator.pin_allocator.buffer
                self.registered_buffer_ptr = buffer.data_ptr()
                # mooncake-integration/store/store_py.cpp::MooncakeDistributedStore::register_buffer(buffer_ptr, size)
                result = self.store.register_buffer(buffer.data_ptr(), buffer.numel())
                if result == 0:
                    """
                    TP=4
                    GLOBAL_SEGMENT_SIZE=53687091200=50GiB
                    MAX_LOCAL_CPU_SIZE=5
                    Registered: 0x7fe0c8000000, 5368709120 bytes=5GiB
                    Registered: 0x7fdbf6000000, 5368709120 bytes=5GiB
                    Registered: 0x7f472c000000, 5368709120 bytes=5GiB
                    Registered: 0x7f99fc0e1000, 5368709120 bytes=5GiB
                    """
                    logger.info(f"Registered: {hex(buffer.data_ptr())}, {buffer.numel()} bytes")
                else:
    ...
    def unregister_cpu_buffer(self):
        """Unregister CPU buffer."""
        if self.registered_buffer_ptr is not None:
            # mooncake-integration/store/store_py.cpp::MooncakeDistributedStore::unregister_buffer(buffer_ptr)
    ...
    def init_chunk_meta(self, config: Optional[LMCacheEngineConfig], metadata: Optional[LMCacheEngineMetadata]) -> None:
        ...
        if self.meta_shape and self.meta_dtype and self.meta_fmt:
            ...
            logger.info("Try setting 'save_chunk_meta' to False in the configuration for better performance")
    ...
    async def put(self, key: CacheEngineKey, memory_obj: MemoryObj):
        ...
        """
        put_with_metadata() 和 put_without_metadata() 有啥区别???
        """

        # Check metadata handling mode to match get behavior
        if self.save_chunk_meta:
            # Use put_parts with metadata stored remotely
            await self.put_with_metadata(key_str, memory_obj)
        else:
            # Use put_from without metadata (zero-copy)
            await self.put_without_metadata(key_str, memory_obj)

    async def put_with_metadata(self, key_str: str, memory_obj: MemoryObj):
        """
        ...
        """
        意思是 metadata + data 也在 buffer 里，即 metadata 也存储在 Mooncake 里。
        且前 METADATA_BYTES_LEN=28 个字节是 metadata，后面是 kv_bytes，即 tensor 的 byte array。
        """
        ...
        try:
            ...
            metadata_bytes = RemoteMetadata(len(kv_bytes), kv_shape, kv_dtype, memory_format).serialize()
            assert len(metadata_bytes) == METADATA_BYTES_LEN

            # mooncake-integration/store/store_py.cpp::MooncakeDistributedStore::put_parts(key, parts, ReplicateConfig)
            # ??? 找了mooncake各个版本，第三个参数不是 kv_bytes，很奇怪!!!
            await asyncio.wait_for(
                asyncio.to_thread(self.store.put_parts, key_str, metadata_bytes, kv_bytes),
                timeout=self.config.transfer_timeout,
            )
        except asyncio.TimeoutError:
            logger.warning(f"Timeout when putting key {key_str} using put_parts. Decode instance may redo prefill.")
        except Exception as e:
            logger.error(f"Failed to put key {key_str} using put_parts: {type(e).__name__}: {str(e)}")
            raise

    async def put_without_metadata(self, key_str: str, memory_obj: MemoryObj):
        """
        ...
        try:
            ...
            # mooncake-integration/store/store_py.cpp::MooncakeDistributedStore::put_from(key, buffer_ptr, size, config)
            await asyncio.wait_for(
                asyncio.to_thread(self.store.put_from, key_str, buffer_ptr, buffer_size, self.replica_config),
                timeout=self.config.transfer_timeout,
            )
        except asyncio.TimeoutError:
            logger.warning(f"Timeout when putting key {key_str} using put_from. Decode instance may redo prefill.")
        except Exception as e:
            logger.error(f"Failed to put key {key_str} using put_from: {type(e).__name__}: {str(e)}")
            raise
    ...
    async def get(self, key: CacheEngineKey) -> Optional[MemoryObj]:
        ...
        raise NotImplementedError("Single get is not supported. Use batched_get([key]) instead.")
    ...
    async def batched_get(self, keys: List[CacheEngineKey]) -> List[Optional[MemoryObj]]:
        ...
        if self.save_chunk_meta:
            ...
            return await self.batch_get_buffer(keys)
        else:
            # Use optimized mode with local metadata
            return await self.batch_get_into(keys)

    async def batch_get_buffer(self, keys: List[CacheEngineKey]) -> List[Optional[MemoryObj]]:
        """
        ...
        for i, buffer in enumerate(buffers):
            if buffer is None:
                ...
                continue
            try:
                memory_obj = self.process_buffer_with_metadata(buffer)
                results.append(memory_obj)
            except Exception as e:
                logger.error(f"Failed to process buffer {i} for key {key_strs[i]}: {str(e)}")
                results.append(None)
        return results

    def process_buffer_with_metadata(self, buffer: bytes) -> Optional[MemoryObj]:
        """
        ...
        metadata = RemoteMetadata.deserialize(bytes(metadata_bytes))
        ...
        """
        意思是从 buffer(metadata+data)里提取出 tensor 数据，然后放到 memory_obj 里。
        """
    ...
    async def batch_get_into(self, keys: List[CacheEngineKey]) -> List[Optional[MemoryObj]]:
        """
        ...
        for i, _ in enumerate(keys):
            buf = self.local_cpu_backend.allocate(self.meta_shape, self.meta_dtype, self.meta_fmt)
        ...
        try:
            ...
            # mooncake-integration/store/store_py.cpp::MooncakeDistributedStore::batch_get_into(keys, buffer_ptrs, sizes)
            # 数据已经读取到 memory_objs[i].tensor 里了
            bytes_read_list = await asyncio.to_thread(self.store.batch_get_into, key_strs, buffer_ptrs, buffer_sizes)
            ...
            for i, n_read in zip(valid_idx, bytes_read_list, strict=False):
                if n_read <= 0: # ???
                    logger.warning(f"batch_get_into failed for key {keys[i]} (code={n_read})")
                    memory_objs[i].ref_count_down()  # type: ignore
                    continue

                try:
                    results[i] = self.reshape_partial_chunk(memory_objs[i], n_read)
                except Exception as exc:
    ...
    async def close(self):
        ...
        self.unregister_cpu_buffer()

        # mooncake-integration/store/store_py.cpp::MooncakeDistributedStore::close()
        self.store.close()
