...
class BaseCachePolicy(Generic[TCache], metaclass=abc.ABCMeta):
    ...
    @abc.abstractmethod
    def update_on_hit(self, key: CacheEngineKey, cache_dict: TCache) -> None:
        """
    ...
    # TODO(Jiayi): we need to unify the `Any` type in the `MutableMapping`
    @abc.abstractmethod
    def get_evict_candidates(self, cache_dict: TCache, num_candidates: int = 1) -> list[CacheEngineKey]:
        """
