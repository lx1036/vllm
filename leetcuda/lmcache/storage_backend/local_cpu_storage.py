...
from leetcuda.lmcache.memory_management import MemoryAllocatorInterface, MemoryFormat, MemoryObj, MixedMemoryAllocator
from leetcuda.lmcache.observability import LMCStatsMonitor, PrometheusLogger
from leetcuda.lmcache.storage_backend.base_backend import AllocatorBackendInterface
from leetcuda.lmcache.storage_backend.cache_policy import get_cache_policy
...
class LocalCPUBackend(AllocatorBackendInterface):
    ...
    def __init__(
        ...
        self.memory_allocator = self.initialize_allocator(config, metadata) if memory_allocator is None else memory_allocator
    ...
    def setup_metrics(self):
        ...
        if prometheus_logger is not None:
            prometheus_logger.local_cpu_hot_cache_count.set_function(lambda: len(self.hot_cache))
            prometheus_logger.local_cpu_keys_in_request_count.set_function(lambda: len(self.keys_in_request))
    ...
    def initialize_allocator(
        ...
        if metadata is not None:
            ...
            save_only_first_rank = config.get_extra_config_value("save_only_first_rank", metadata.use_mla) and metadata.use_mla
            if save_only_first_rank and metadata.is_first_rank():
                ...
                cpu_size = config.extra_config.get("first_rank_max_local_cpu_size", cpu_size) if config.extra_config else cpu_size
        ...
        if config.enable_p2p:
            ...
            paged_memory_allocator = PagedCpuGpuMemoryAllocator()
            paged_memory_allocator.init_cpu_memory_allocator(
                int(cpu_size * 1024**3),
                shape=new_shape,
    ...
    def allocate(
        ...
        while True:
            ...
            num_attempts += 1
            logger.debug(f"Unable to allocate memory object after {num_attempts} attempts of local cpu backend allocate()")
    ...
    def batched_allocate(
        ...
        regardless of whether local_cpu is True or False
        """
        logger.debug(f"Batched allocating memory in local cpu backend with busy loop: {busy_loop}")
        if fmt is None:
            if self.layerwise:
                if self.enable_blending:
                    fmt = MemoryFormat.KV_2TD
        ...
        while True:
            ...
            num_attempts += 1
            logger.debug(f"Unable to allocate memory object after {num_attempts} attempts of local cpu backend batched_allocate()")
    ...
    def calculate_chunk_budget(self) -> int:
        ...
        assert isinstance(self.memory_allocator, MixedMemoryAllocator)
        assert self.metadata is not None, "metadata required for chunk budget calculation"
    ...
    def submit_put_task(self, key: CacheEngineKey, memory_obj: MemoryObj) -> Optional[Future]:
        ...
        with self.cpu_lock:
            ...
            if self.lmcache_worker is not None:
                self.lmcache_worker.put_msg(KVAdmitMsg(self.instance_id, key.worker_id, key.chunk_hash, str(self)))
        return None
    ...
    def remove(self, key: CacheEngineKey, force: bool = True) -> bool:
        ...
        if self.lmcache_worker is not None:
            self.lmcache_worker.put_msg(KVEvictMsg(self.instance_id, key.worker_id, key.chunk_hash, str(self)))
